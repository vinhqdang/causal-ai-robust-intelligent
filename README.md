...
**🆕 Latest Updates (v2.1)** - Enhanced ICLR 2026 Ready Implementation:
- ✅ **NOTEARS Acyclicity Enforcement** - Guarantees DAG property with cycle-breaking optimization
- ✅ **Importance-Sampled Path Integration** - Efficient O(E × 20) path enumeration vs O(E!) naive approach
- ✅ **Dynamic Tucker Rank Selection** - Adaptive rank adjustment based on 95% explained variance
- ✅ **Curriculum KL Annealing** - Stabilized diffusion training with multiple annealing schedules
- ✅ **Stick-Breaking VAE Alternative** - End-to-end differentiable routing with continuous relaxation
- ✅ **PAC-Bayes Empirical Validation** - Comprehensive validation suite with √T risk verification
...
| Component | Purpose | Key Innovation | v2.1 Enhancement |
|-----------|---------|----------------|------------------|
| **OGR** | Dynamic graph learning | Kernelised CI tests on latent variables | ✨ NOTEARS acyclicity penalty |
| **AFS** | Memory-efficient adapters | Tucker-2 factorization: A_e = B ×₁ u_e^(1) ×₂ u_e^(2) | ✨ Dynamic rank selection (95% variance) |
| **PI-CPI** | Path-aware regularization | ∫₀¹ ∂_θⱼ τₚ(θ₀ + αΔθⱼ) dα across causal paths | ✨ Importance sampling (≤20 paths/edge) |
| **GCR** | High-fidelity replay | Lightweight diffusion UNet with causal conditioning | ✨ Curriculum KL annealing + tiktoken |
| **BUG** | Uncertainty-aware routing | Dirichlet posterior with Thompson sampling | ✨ Stick-Breaking VAE alternative |
...
# Configure CASCADA with v2.1 enhancements
config = CASCADAConfig(
    ...
    # v2.1 Enhanced Parameters
    notears_lambda=1.0,           # NOTEARS acyclicity penalty weight
    target_variance_ratio=0.95,   # Dynamic rank selection threshold
    max_paths_per_edge=20,        # Importance sampling limit
    kl_annealing_schedule="cosine", # KL annealing: "linear", "cosine", "exponential", "cyclical"
    router_type="stick_breaking_vae"  # BUG router: "dirichlet" or "stick_breaking_vae"
)
...
### 🧬 PAC-Bayes Validation (New!)

Validate the theoretical PAC-Bayes bounds against empirical performance:

```bash
# Run comprehensive PAC-Bayes validation
python validate_pac_bayes.py --complexity medium --trials 10

# Custom configuration
python validate_pac_bayes.py \
    --config pac_bayes_config.json \
    --output ./pac_bayes_results \
    --complexity large \
    --trials 20
```

This will generate:
- **Risk curves** showing empirical vs theoretical bounds
- **√T dependence analysis** with R² fit quality
- **Violation rate statistics** across confidence levels
- **Comprehensive validation report** with interpretations
